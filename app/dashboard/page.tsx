           ...
           <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700 mt-4">
              <div className="bg-slate-900 text-white p-6 rounded-2xl shadow-xl text-center py-10 relative overflow-hidden">
                 ...
                 <div className="relative z-10">
                    ...
                    <Link href="/dashboard/invite" className="bg-blue-600 hover:bg-blue-500 text-white font-bold py-3 px-8 rounded-xl inline-flex items-center gap-2 transition-all active:scale-95 shadow-lg shadow-blue-900/50"><QrCode size={20} /> Invite Student</Link>
                 </div>
              </div>
              <div className="grid grid-cols-2 gap-3">
                  <Link href="/dashboard/help" className="flex items-center justify-center gap-2 p-3 bg-white border border-slate-200 rounded-xl text-slate-600 font-bold text-xs hover:border-blue-300 transition-colors"><BookOpen size={16} /> How to Use</Link>
                  <Link href="/dashboard/install" className="flex items-center justify-center gap-2 p-3 bg-white border border-slate-200 rounded-xl text-slate-600 font-bold text-xs hover:border-blue-300 transition-colors"><Download size={16} /> Install App</Link>
              </div>
           </div>
        ) : (
           <>
                ...
                <div className="flex gap-2">
                    <div className="relative flex-1">
                        ...
                        <input type="text" placeholder="Search name..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full pl-9 pr-4 py-2.5 rounded-xl bg-white border border-slate-200 text-sm font-bold text-slate-900 placeholder:text-slate-400 focus:ring-2 focus:ring-blue-500 outline-none" />
                    </div>
                            ...
                            <Link key={student.id} href={`/dashboard/student/${student.id}`} className="block bg-white p-4 rounded-xl border border-slate-100 shadow-sm active:scale-[0.98] transition-transform group hover:border-blue-300">
                                <div className="flex justify-between items-center">
                                    <div className="flex items-center gap-4">
                                        <div className={`w-12 h-12 rounded-full flex items-center justify-center font-bold text-lg ${student.archived ? 'bg-slate-100 text-slate-400' : 'bg-blue-100 text-blue-600'}`}>{student.full_name.charAt(0)}</div>
                                        <div>
                                    ...
                                    <div className="flex items-center gap-3">
                                        {student.unpaid_count > 0 && !student.archived && (
                                            <div className="bg-red-50 text-red-600 border border-red-100 px-2.5 py-1 rounded-md text-[10px] font-extrabold uppercase tracking-wide flex items-center gap-1"><AlertCircle size={10} />{student.unpaid_count} Unpaid</div>
                                        )}
